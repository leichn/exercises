...
/* 如下注释 1. 和 2. 两步是 PIMPL 技法，用于解开类的接口与实现的耦合，减少编译依赖
   ...
   PIMPL 技法在编译解除了类的接口与实现的耦合，减少了编译依赖。在本例中，如果类 Ui::MainWindow 的定义有变化，不会引起
   包含了 mainwindow.h 的源文件的重新编译。
*/
...
private:
    ...
    // Ui::MainWindow 定义的改变只会引起使用了 Ui::MainWindow 类的源文件重新编译，而不会引起其他包含本头文件的源文
    // 件重新编译。即：本头文件 (MainWindow 类) 与 Ui::MainWindow 类解除了耦合。
    Ui::MainWindow *ui;
