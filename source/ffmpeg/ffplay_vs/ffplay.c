...
static void frame_queue_destory(FrameQueue *f)
{
    ...
    for (i = 0; i < f->max_size; i++) {
        Frame *vp = &f->queue[i];
        frame_queue_unref_item(vp);     // 释放对frame中的数据缓冲区的引用，注意不是释放frame对象本身
        av_frame_free(&vp->frame);      // 释放frame对象
    }
...
{
    packet_queue_start(d->queue);   // 启动播放线程前启动packet_queue
...
static int video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        while (ret >= 0) {
            ...
            ret = queue_picture(is, frame, pts, duration, frame->pkt_pos, is->viddec.pkt_serial);   // 将当前帧压入frame_queue
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        nb_channels    = avctx->channels;           // 声道数
        channel_layout = avctx->channel_layout;     // 声道布局，在audio_open()中说明
#endif
